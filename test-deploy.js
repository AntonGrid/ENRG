...
(async () => {
  ...
  if (!vaultInfo) {
    const ivIx = new TransactionInstruction({
      ...
      programId,
      data: getDisc("initialize_vault")
    });
    await require("@solana/web3.js").sendAndConfirmTransaction(connection, new Transaction().add(ivIx), [walletKeypair]);
  ...
  if (!buybackInfo) {
    const ifIx = new TransactionInstruction({
      ...
      programId,
      data: getDisc("initialize_funds")
    });
    await require("@solana/web3.js").sendAndConfirmTransaction(connection, new Transaction().add(ifIx), [walletKeypair]);
  ...
  if (!producerInfo) {
    ...
    const cpData = Buffer.alloc(40);
    getDisc("create_producer").copy(cpData);
    ...
    await require("@solana/web3.js").sendAndConfirmTransaction(connection, new Transaction().add(cpIx), [walletKeypair]);
    console.log("Producer created:", producerPda.toBase58());
  } else {
    console.log("Producer already exists, but authority might differ. Skipping.");
  }
