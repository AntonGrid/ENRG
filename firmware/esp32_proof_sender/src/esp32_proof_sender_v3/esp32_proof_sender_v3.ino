...
// Issuer chain: onrender.com ← GTS WE1 ← GTS Root R4 (https://pki.goog/repo/certs/gtsr4.pem).
...
#if ENRG_USE_SE050
// NXP SE050 (Plug & Trust): SSS API.
//
// These headers come from NXP's Plug & Trust middleware — vendored, NOT a
// PlatformIO registry package (the old `lib_deps = se050` entry never resolved:
// "Could not find the package with 'se050' requirements"). Fetch it with
// scripts/vendor-se050.sh, which also records the pinned upstream ref; see
// SE050-HARDWARE-SIGNING.md §Vendoring and lib/README.md.
//
...
#endif
#include <sss.h>                 // umbrella generated by the vendor script
#include <fsl_sss_se05x_apis.h>  // sss_se05x_* (the concrete implementation)
#include <fsl_sss_se05x_types.h> // sss_se05x_session_t / _key_store_t / _object_t ...
#include <nxScp03_Types.h>       // SE_Connect_Ctx_t — the session's connection ctx
#include <enrg_se050_port.h>     // lamp: pulls lib/enrg_se050_port into the build
#endif
...
//
//  ⚠️ STATUS: compiles and links against the pinned middleware
//  (lib/se05x/VERSION) with the ESP32 host port in lib/enrg_se050_port, but no
//  board with an SE050 has ever been connected — the tier is NOT verified on
//  silicon (docs/STATE.md). Two things must be true for it to work, and neither
//  is checkable without the chip:
//    1. the T=1-over-I2C framing of the port matches the chip (see the port's
//       header comment), and
//    2. the applet has EDDSA enabled (`AppletConfig_EDDSA`; curve
//       RESERVED_ID_ECC_ED_25519) — a factory/OTP-level setting.
//
//  API note for reviewers: this section used to call symbols that do not exist
//  in the NXP middleware (`sss_se05x_connect`, `sss_open_session`,
//  `sss_crypto_object_create`, `kSSS_KeyPart_Pair_Ed25519`,
//  `kSSS_CipherType_EC_ED25519`, ...), which is why the tier could never build.
//  The calls below are the real SSS API:
//    sss_se05x_session_open / _key_store_context_init / _key_object_init
//    sss_se05x_key_object_{allocate_handle,get_handle}
//    sss_se05x_key_store_generate_key / _get_key
//    sss_se05x_asymmetric_context_init / _sign
//  Ed25519 is kSSS_CipherType_EC_TWISTED_ED + kAlgorithm_SSS_SHA512 (the sign
//  path maps that to the chip's kSE05x_EDSignatureAlgo_ED25519PURE_SHA_512).
// ════════════════════════════════════════════════════════════════

static SE_Connect_Ctx_t g_se05x_conn = {};
static sss_se05x_session_t g_se05x_session = {};
static sss_se05x_key_store_t g_se05x_key_store = {};
static sss_se05x_object_t g_se05x_key_obj = {};
static sss_se05x_asymmetric_t g_se05x_asym = {};
static bool g_se050_ready = false;

/** Open an SSS session over T=1/I2C (the transport comes from lib/enrg_se050_port). */
static bool se050_open() {
    g_se05x_conn.auth.authType = kSSS_AuthType_None;
    g_se05x_conn.connType = kType_SE_Conn_Type_T1oI2C;
    g_se05x_conn.portName = "t1oi2c"; // the ESP32 port ignores it: the bus is fixed
    g_se05x_conn.i2cAddress = ENRG_SE050_I2C_ADDR;
    sss_status_t st = sss_se05x_session_open(&g_se05x_session, kType_SSS_SE_SE05x, 0,
                                             kSSS_ConnectionType_Plain, &g_se05x_conn);
    if (st != kStatus_SSS_Success) {
        Serial.println("[SE050] session open failed");
        return false;
    }
    st = sss_se05x_key_store_context_init(&g_se05x_key_store, &g_se05x_session);
    if (st != kStatus_SSS_Success) {
        Serial.println("[SE050] key store init failed");
        return false;
    }
    st = sss_se05x_key_object_init(&g_se05x_key_obj, &g_se05x_key_store);
    if (st != kStatus_SSS_Success) {
 ...
 *
 * `keyId` = ENRG_SE050_KEY_ID. Note the chip's public key may come back DER-wrapped
 * (12-byte header for the twisted-Edwards curve), so both lengths are accepted.
 */
static bool se050_load_or_create_key(uint8_t publicKey[32]) {
    sss_status_t st = sss_se05x_key_object_get_handle(&g_se05x_key_obj, ENRG_SE050_KEY_ID);
    if (st != kStatus_SSS_Success) {
        // No key yet — allocate the object and let the CHIP generate the pair
        // (the secret never appears on the bus, only its public half does).
        st = sss_se05x_key_object_allocate_handle(&g_se05x_key_obj, ENRG_SE050_KEY_ID,
                                                 kSSS_KeyPart_Pair,
                                                 kSSS_CipherType_EC_TWISTED_ED,
                                                 32, kKeyObject_Mode_Persistent);
        if (st != kStatus_SSS_Success) {
            Serial.println("[SE050] key allocate failed");
            return false;
        }
        st = sss_se05x_key_store_generate_key(&g_se05x_key_store, &g_se05x_key_obj, 256, NULL);
        if (st != kStatus_SSS_Success) {
            Serial.println("[SE050] key generate failed");
            return false;
        }
    ...
    // kAlgorithm_SSS_SHA512 is what the SE05x sign path requires for Ed25519
    // (it selects kSE05x_EDSignatureAlgo_ED25519PURE_SHA_512 internally).
    st = sss_se05x_asymmetric_context_init(&g_se05x_asym, &g_se05x_session,
                                           &g_se05x_key_obj, kAlgorithm_SSS_SHA512,
                                           kMode_SSS_Sign);
    if (st != kStatus_SSS_Success) {
    ...
    size_t keyBits = 0;
    st = sss_se05x_key_store_get_key(&g_se05x_key_store, &g_se05x_key_obj, keyBuf, &keyLen, &keyBits);
    if (st != kStatus_SSS_Success) {
        Serial.println("[SE050] get pub key failed");
        return false;
    }
    const uint8_t *raw = keyBuf;
    if (keyLen == 44) {
        raw = keyBuf + 12; // DER header for the twisted-Edwards curve
        keyLen = 32;
    }
    if (keyLen != 32) {
        Serial.printf("[SE050] unexpected pub key length %u\n", (unsigned)keyLen);
        return false;
    }
    memcpy(publicKey, raw, 32);
    return true;
}
...
static bool se050_sign(const uint8_t *msg, size_t msgLen, uint8_t signature[64]) {
    size_t sigLen = 64;
    /* SE05x signs the raw message: kAlgorithm_SSS_SHA512 in the context selects
     * Ed25519 (pure, SHA-512 internally per RFC 8032) — no pre-hashing here. */
    sss_status_t st = sss_se05x_asymmetric_sign(&g_se05x_asym, msg, msgLen, signature, &sigLen);
