...
| CI | 🟡 5 of 6 jobs green | Node, Python, Rust, Foundry and gitleaks pass on every push. The `anchor test` job was red on 30+ runs since 2026-08-31 for a reason nobody could see: it reads three key files that only exist on a developer machine — `target/deploy/enrg_mvp-keypair.json` (gitignored, so `anchor build` picked a random program id while the suite pins `HkuC3FT…`), `~/.config/solana/id.json` (the `[provider] wallet`, absent from any fresh runner → `Error: Unable to read keypair file`) and `~/.config/solana/founder-wallet.json` (loaded by six suites at import time → `ENOENT` before a single test ran). Reproduced locally by moving each file aside. CI now creates the throwaway provider wallet itself, restores the program keypair from the `ENRG_MVP_PROGRAM_KEYPAIR` secret (set) and the founder key from `ENRG_FOUNDER_WALLET` (pending — the job fails with that instruction, not a mystery). With all three in place the whole suite passes in a CI-like `HOME`: 55 passing / 4 pending |
