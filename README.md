...
| The key lives inside an NXP SE050 and never leaves the chip | ⚠️ **tier compiles; chip never flashed** | Path is real code behind `ENRG_USE_SE050`, and the tier now **builds**: `pio run -e esp32dev-se050` succeeds (20 s, 93.6% of the 1.3 MB app slot) against the vendored NXP Plug & Trust middleware — BSD-3-Clause, pinned ref in `lib/se05x/VERSION`, fetched by `firmware/esp32_proof_sender/scripts/vendor-se050.sh` — plus our own ESP32 host port (`lib/enrg_se050_port/`: I2C PAL over T=1, timers, reset hooks). Two earlier claims were wrong and are corrected: the middleware is **not** closed-licence, and the firmware called SSS functions that do not exist in the NXP API (`sss_se05x_connect`, `sss_crypto_object_create`, `kSSS_KeyPart_Pair_Ed25519`, …), which is why it could never build. What remains is the chip: no board with an SE050 has been connected, so no proof has been signed on silicon — bring-up needs the board, EDDSA enabled in the applet, and a logic analyser for the first frames |
...
| CI | 🟢 5 of 5 jobs green | `ci.yml` runs Python, Rust, Node (with the SDK and wire-vector checks), Foundry and the read-only authority guard — all green on every push; gitleaks runs as its own workflow (`secrets.yml`). The Anchor integration suite is deliberately **not** run in CI (decision 2026-09-21): it needs three key files that exist only on a developer machine — the gitignored `target/deploy/enrg_mvp-keypair.json`, the `[provider] wallet` `~/.config/solana/id.json`, and `~/.config/solana/founder-wallet.json` — and the last one is the founder key (`EXPECTED_DEPLOYER = FnqKH4…`), which must not be handed to CI, since repository secrets reach pull requests opened from branches of this repository. It was red for 30+ runs since 2026-08-31 precisely because nobody saw those three files; now it runs locally: `anchor test` → 55 passing / 4 pending. CI still compiles and unit-tests the program (Rust job, 125 tests) and keeps the authority guard |
