...
impl FounderVesting {
    ...
    /// Amount available for claim at time `now` (cliff + linear release).
    pub fn vested_at(&self, now: i64) -> u64 {
        vested_at(self.total_amount, self.start_time, self.cliff, self.release, now)
    }
}

/// Pure vesting calculation function (extracted for unit tests).
///
/// Rules (fixed economic model):
/// - until `start_time + cliff` — 0 (fully locked);
/// - then linear over `release` (on average 1/36 per month when
///   release = 3 years);
/// - after `start_time + cliff + release` — the whole amount is unlocked.
...
mod tests {
    ...
    fn cliff_blocks_everything() {
        // 1. elapsed < cliff → 0 (fully locked, not a single coin).
        ...
        // At the very start (now == start_time) — also 0.
        assert_eq!(
            vested_at(
                FOUNDER_ALLOCATION_ATOMIC,
                START,
    ...
    fn cliff_boundary_is_exclusive() {
        // 2. elapsed == cliff → still 0 (boundary is exclusive).
    ...
    fn full_cycle_unlocks_everything() {
        // 4. elapsed == cliff + release → everything unlocked; no further growth.
    ...
    fn one_month_vests_about_1_36() {
        // 5. elapsed == cliff + 1 month (30 days) → ≈ 1/36 of the amount.
        ...
        // Exact floor value of the linear calculation.
        let month_exact = ((FOUNDER_ALLOCATION_ATOMIC as u128 * month_secs as u128)
            / FOUNDER_VESTING_RELEASE as u128) as u64;
        assert_eq!(vested, month_exact, "rounding is floor, no losses");
        assert!(vested > 0);

        // "1/36 per month" is an approximation: the difference is at most 2%.
        let one_36 = FOUNDER_ALLOCATION_ATOMIC / 36;
        let diff = vested.abs_diff(one_36);
        assert!(diff <= FOUNDER_ALLOCATION_ATOMIC / 50, "diff={} (tolerance 2%)", diff);
    }

    #[test]
    fn premine_fits_supply_cap() {
        // 6. total_supply + 2e17 <= MAX_SUPPLY_ATOMIC (at genesis total=0).
        assert_eq!(FOUNDER_ALLOCATION_ATOMIC, 200_000_000_000_000_000); // 20% of 1e18
    ...
    fn supply_after_premine_stays_within_cap() {
        // 9. Emission after premine (total=2e17) does not exceed MAX_SUPPLY_ATOMIC.
    ...
    fn emission_math_sane_after_premine() {
        // 4.2: energy_per_src(2e17) > 0, reward without division by zero/overflow.
    ...
    fn discriminator_and_len_lock_genesis_layout() {
        // The genesis account (tests/genesis/founder-vesting.json) stores
        // 88 bytes = 8 (discriminator) + 80 (LEN). Any change to the structure
        // of FounderVesting or the account name would break the local genesis
        // injection and the bootstrap path — this test locks the layout.
        use anchor_lang::Discriminator;
        assert_eq!(
            FounderVesting::DISCRIMINATOR,
            [0x5e, 0x95, 0x78, 0x1f, 0x24, 0x17, 0x0e, 0xa4],
            "FounderVesting discriminator must match the genesis file"
        );
    ...
    fn claim_transfer_moves_claimable_only() {
        // 8. Simulate two claims: the founder ATA decreases by exactly claimable,
        // the destination ATA increases by claimable; total is fully unlocked.
        let total = FOUNDER_ALLOCATION_ATOMIC;
        let mut founder_balance = total; // after premine
